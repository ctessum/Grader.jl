...
@testset "Grader.jl" begin
    ...
    @testset "evalasmodule" begin
        ...
        golden_result = evalasmodule(golden_code)
        student_result = evalasmodule(student_code)
        ...
        @testset "Throws ParseError for syntax error" begin
            syntaxerr = "x= @@@"
            @test_throws Base.Meta.ParseError evalasmodule(syntaxerr)
        end
    end
    ...
    @testset "grade problem" begin
        @testset "correct answer" begin
            ...
            p = Problem()
            golden, student = run!(p, goldencode, studentcode)
